...
async function discover(params, options = {}) {
  ...
  let resultParams;
  return sanitizeDiscoverParams(params).then(sanitizedParams => {
...
async function getImageFormat(idOrName) {
  ...
  imageConstants.formats.every(format => {
    if (typeof idOrName === 'string' && format.name === idOrName || Number.isInteger(idOrName) && format.id === idOrName) {
      result = format;
      return false;
    }
...
async function getDiscography(artistOrLabelUrl, options = {}) {
  ...
  return _fetchPage(utils.getUrl('music', artistOrLabelUrl)).then(html => parser.parseDiscography(html, opts));
}
...
async function getArtistOrLabelInfo(artistOrLabelUrl, options = {}) {
  ...
  return _fetchPage(utils.getUrl('music', artistOrLabelUrl)).then(html => parser.parseArtistOrLabelInfo(html, opts)).then(info => {
    if (info.type === 'label' || info.name !== '') {
      return info;
    } else {
      return getDiscography(artistOrLabelUrl, options).then(discographyItems => {
      ...
      }).then(url => _fetchPage(url)).then(html => parser.parseArtistOrLabelInfo(html, opts));
    }
  });
}
...
async function getLabelArtists(labelUrl, options = {}) {
  ...
  return _fetchPage(utils.getUrl('artists', labelUrl)).then(html => parser.parseLabelArtists(html, opts));
}
...
async function search(params, options = {}) {
  ...
  return _fetchPage(utils.getSearchUrl(params)).then(html => parser.parseSearchResults(html, opts));
}
...
async function getAlbumHighlightsByTag(tagUrl, options = {}) {
  ...
  return _fetchPage(tagUrl).then(html => parser.parseAlbumHighlightsByTag(html, opts));
}

async function getTags() {
  return _fetchPage(utils.getUrl('tags')).then(html => parser.parseTags(html));
}
...
async function getAllShows(options = {}) {
  ...
  return _fetchPage(utils.getAllShowsUrl(), true).then(json => parser.parseAllShows(json, opts));
}
...
async function getShow(showUrl, options = {}) {
  ...
  return _fetchPage(showUrl).then(html => parser.parseShow(html, opts));
}

async function getArticleCategories() {
  return _fetchPage(utils.getDailyUrl()).then(html => parser.parseArticleCategories(html));
}
...
async function getArticleList(params = {}, options = {}) {
  ...
  return _fetchPage(utils.getDailyUrl(params)).then(html => parser.parseArticleList(html, opts));
}
...
async function getArticle(articleUrl, options = {}) {
  ...
  return _fetchPage(articleUrl).then(html => parser.parseArticle(html, opts));
}

async function getTagInfo(tagUrl) {
  return _fetchPage(tagUrl).then(html => parser.parseTagInfo(html, {
    tagUrl
  }));
}

async function getReleasesByTagFilterOptions(tagUrl) {
  return getReleasesByTagFilterValueNames(tagUrl).then(filterValueNames => {
    const opts = {
      filterValueNames
    };
    return _fetchPage(tagUrl).then(html => parser.parseReleasesByTagFilterOptions(html, opts));
  });
}

async function getReleasesByTagFilterValueNames(tagUrl) {
  return _fetchPage(utils.getReleasesByTagUrl(tagUrl)).then(html => parser.parseHubJSPath(html)).then(path => {
...
async function getReleasesByTag(tagUrl, params = {}, options = {}) {
  ...
  const _getDefaultFilters = tagUrl => {
    if (options.useHardcodedDefaultFilters) {
      ...
      return getReleasesByTagFilterOptions(tagUrl).then(filterOptions => {
  ...
  return _getDefaultFilters(tagUrl).then(defaultFilters => {
    ...
    const paramFilters = params.filters ? Object.assign(defaultFilters, params.filters, {
      tags: tagsFilter
    }) : defaultFilters;
    return {
    ...
    return _fetchPage(utils.getDigDeeperUrl(), true, _getPostFetchOptions(postData)).then(json => parser.parseReleasesByTag(json, opts));
  });
}
...
async function searchTag(params) {
  ...
  return _fetchPage(utils.getSearchTagUrl(), true, _getPostFetchOptions(postData)).then(json => parser.parseSearchTagResults(json));
}
...
async function searchLocation(params) {
  ...
  return _fetchPage(utils.getSearchLocationUrl(), true, _getPostFetchOptions(postData)).then(json => parser.parseSearchLocationResults(json));
}
...
module.exports = Object.assign({}, _exportFn, {
  cache,
  limiter
});