...
function parseDiscoverOptions(html) {
  ...
  if (typeof parsed === 'object' && typeof parsed.discover_2015 === 'object' && typeof parsed.discover_2015.options === 'object') {
...
function parseAlbumHighlightsByTag(html, opts) {
  ...
  if (typeof parsed === 'object' && parsed.hub && parsed.hub.tabs && parsed.hub.tabs[0].collections) {
...
function parseTags(html) {
  ...
  const _parseCloud = id => {
    ...
    cloud.find('a.tag').each((index, link) => {
      ...
      if (name && link.attr('href') !== '/tag/' && !_findTag(url, name, tagsInCloud)) {
        tagsInCloud.push({
          name,
          url
        });
  ...
  if (tObj[1]) {
    ...
    if (t && t[0] && Array.isArray(t[0].blocks)) {
      ...
      t[0].blocks.forEach(filterBlock => {
        ...
        if (filter) {
          filterBlock.attachment.find(a => a.blocks).blocks.filter(block => block.expression).forEach(valueBlock => {
