...
async function search(query, message, _once) {
  ...
  for (let i = 0; i < search.length; i++) {
    const song = search[i];
    const title = platform === 'sc' || platform === 'yt' ? song.title : song.name;
    const artist = platform === 'sc' ? song.user.username : platform === 'bc' ? song.artist : song.author.name;
    const url = platform === 'sc' ? song.permalink_url : song.url;
    const thumbnail = platform === 'sc' ? song.artwork_url : platform === 'bc' ? song.imageUrl : song.bestThumbnail.url;
    const duration = platform === 'sc' ? require('../../etc/utils').time_format(Math.round(song.full_duration / 1000)) : platform === 'yt' ? song.duration : require('../../etc/utils').time_format(Math.round((await require('bandcamp-fetch').getTrackInfo(song.url)).duration));
    const track = {
      title: title,
  ...
  message.channel.send(embed).then(recent => {
    ...
    recent.channel.awaitMessages(m => m.author.id == message.author.id, {
      ...
      if (/^-?\d+$/.test(collected.first().content) && collected.first().content > 0 && collected.first().content < search.length + 1) {
        ...
        if (!message.member.voice.channel) message.channel.send(`Please join a voice channel first!`);else require('./player').play(platform, track, message.member.voice.channel, message.channel);
      }
